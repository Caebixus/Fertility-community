...
def fertilityClinicBerlin(request):
    guestblog = GuestBlog.objects.filter(guestblogcity__iexact='Berlin')
    guestblog = guestblog.filter(guestblogactive=True)

    all_clinic_count = BasicClinic.objects.filter(is_published=True)
    all_clinic_count = all_clinic_count.count()

    average = BasicClinic.objects.filter(clinicState__iexact='Germany')
    averageIVFPrice = average.aggregate(average=Avg('ovarian_ivf_treatment_cost'))
    averageEggPrice = average.aggregate(average=Avg('egg_donor_recipients_cost'))
    averageEmbryoPrice = average.aggregate(average=Avg('embryo_donor_recipients_cost'))
    averageSpermPrice = average.aggregate(average=Avg('sperm_donor_recipients_cost'))
    ...
    basic_queryset = BasicClinic.objects.filter(clinicRegion__iexact='Berlin').filter(is_published=True)
    ...
    return render(request, 'locations-cities/Europe/Germany/fertility-clinics-berlin.html', context)


def fertilityClinicLisbon(request):
    guestblog = GuestBlog.objects.filter(guestblogcity__iexact='Lisbon')
    guestblog = guestblog.filter(guestblogactive=True)

    all_clinic_count = BasicClinic.objects.filter(is_published=True)
    all_clinic_count = all_clinic_count.count()

    average = BasicClinic.objects.filter(clinicState__iexact='Portugal')
    averageIVFPrice = average.aggregate(average=Avg('ovarian_ivf_treatment_cost'))
    averageEggPrice = average.aggregate(average=Avg('egg_donor_recipients_cost'))
    averageEmbryoPrice = average.aggregate(average=Avg('embryo_donor_recipients_cost'))
    averageSpermPrice = average.aggregate(average=Avg('sperm_donor_recipients_cost'))
    ...
    basic_queryset = BasicClinic.objects.filter(clinicRegion__iexact='Lisbon').filter(is_published=True)
    ...
    return render(request, 'locations-cities/Europe/Portugal/fertility-clinics-lisbon.html', context)


def fertilityClinicRiga(request):
    guestblog = GuestBlog.objects.filter(guestblogcity__iexact='Riga')
    guestblog = guestblog.filter(guestblogactive=True)

    all_clinic_count = BasicClinic.objects.filter(is_published=True)
    all_clinic_count = all_clinic_count.count()

    average = BasicClinic.objects.filter(clinicState__iexact='Latvia')
    averageIVFPrice = average.aggregate(average=Avg('ovarian_ivf_treatment_cost'))
    averageEggPrice = average.aggregate(average=Avg('egg_donor_recipients_cost'))
    averageEmbryoPrice = average.aggregate(average=Avg('embryo_donor_recipients_cost'))
    averageSpermPrice = average.aggregate(average=Avg('sperm_donor_recipients_cost'))
    ...
    basic_queryset = BasicClinic.objects.filter(clinicRegion__iexact='Riga').filter(is_published=True)
    ...
    return render(request, 'locations-cities/Europe/Latvia/fertility-clinics-riga.html', context)
